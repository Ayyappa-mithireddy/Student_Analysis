...
# Endpoint 1: Return overall academic summary
@app.get("/summary")
def get_summary():
    # Calculate total number of students
    total_students = len(df)

    # Exclude dropouts (G3 = 0) for academic performance calculations
    non_dropout = df[df["G3"] != 0]

    # Calculate average G3 among non-dropout students
    class_average_g3 = round(float(non_dropout["G3"].mean()), 2)

    # Calculate pass rate among non-dropout students
    # A student passes when G3 >= 10
    pass_rate_percent = round(
        float((non_dropout["G3"] >= 10).mean() * 100), 2
    )

    # Count students at risk (G3 from 1 to 9)
...
# Endpoint 2: Return students who are at risk of failing
@app.get("/at-risk")
def get_at_risk():
    ...
    # Sort by G3 ascending so the worst-performing students appear first
    ...
    # Convert the DataFrame to a list of JSON-compatible dictionaries
...
class StudentInput(BaseModel):
    # G1 validation: must be between 0 and 20
    G1: float = Field(
        ...,
        ge=0,
        le=20,
        description="G1 grade must be between 0 and 20"
    )

    # G2 validation: must be between 0 and 20
    G2: float = Field(
        ...,
        ge=0,
        le=20,
        description="G2 grade must be between 0 and 20"
    )

    # Study time validation: must be between 1 and 4
    studytime: int = Field(
        ...,
        ge=1,
        le=4,
    ...
    # Absences validation: must be between 0 and 100
    absences: int = Field(
        ...,
        ge=0,
        le=100,
    ...
    # Failures validation: must be between 0 and 4
    failures: int = Field(
        ...,
        ge=0,
        le=4,
...
# POST /predict-result endpoint
@app.post("/predict-result")
def predict_result(student: StudentInput):
    # Calculate the estimated G3 using the specified formula
    estimated_g3 = (
        (student.G1 * 0.3)
        + (student.G2 * 0.6)
    ...
    # Clamp the estimated G3 value between 0 and 20
    estimated_g3 = max(0, min(20, estimated_g3))

    # Determine the predicted result based on estimated G3
    if estimated_g3 == 0:
...
# Main block: Start the FastAPI server using Uvicorn
